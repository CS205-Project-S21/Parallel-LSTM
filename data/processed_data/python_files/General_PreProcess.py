...
def main():
    stockprice_rawdata_path_COG = '../../stock_price_Energy/data/price_COG.csv'
    stockprice_rawdata_path_DVN = '../../stock_price_Energy/data/price_DVN.csv'
    stockprice_rawdata_path_HFC = '../../stock_price_Energy/data/price_HFC.csv'
    ...
    df_price = df_COG.join((df_DVN.join(df_HFC, on=['window'], how='left_outer')), on=['window'], how='left_outer')
    df_final = df_news.join(df_price, on=['window'], how='left_outer')

    df_final.orderBy('window').toPandas().to_csv('processed_data_energy.csv', index=False)
